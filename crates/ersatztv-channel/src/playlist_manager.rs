...
mod tests {
    ...
    /// A manager holding `segment_count` four-second segments, the first
    /// starting at `channel_start`, with the live edge advanced exactly as
    /// `update` advances it.
    fn manager(segment_count: u64, channel_start: OffsetDateTime) -> PlaylistManager {
        ...
        for i in 0..segment_count {
            ...
            m.last_segment_end = channel_start + Duration::from_secs((i + 1) * 4);
        }
    ...
    fn keeps_two_minutes_of_media_behind_the_live_edge() {
        ...
        let m = manager(40, OffsetDateTime::UNIX_EPOCH);
    ...
    fn history_survives_a_channel_running_behind_the_wall_clock() {
        ...
        let on_time = manager(40, OffsetDateTime::UNIX_EPOCH);
        let behind = manager(
            40,
    ...
    fn nothing_is_trimmed_before_the_window_fills() {
        ...
        let m = manager(10, OffsetDateTime::UNIX_EPOCH);
