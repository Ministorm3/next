...
mod tests {
    ...
    /// Segments are immutable, so nothing here changes their handling.
    #[tokio::test]
    ...
    /// The content type fix keeps its existing scope: media playlists get
    /// the HLS type, and nothing else is retyped.
    #[tokio::test]
    async fn only_a_media_playlist_is_retyped() {
